...
DAGEdges compute_dag_edges_off_chain(Vertex* col_vtx, HalfEdge* col_edge, HalfEdge* next_col_edge){
	...
	// Computing spes[2]
	path_weight_on_chain = next_col_edge->apex.first->v_chain[index1].second - next_col_edge->apex.first->v_chain[index2].second; 
	...
	// Computing spes[3]
	path_weight_on_chain = next_col_edge->apex.first->v_chain[index1].second - next_col_edge->apex.first->v_chain[index2 + 1].second;
...
void compute_dag_edges(DAG& dag_graph, HalfEdge* dag, HalfEdge* next_dag, Vertex* sink){
...
DAG build_dag(Vertex* src, Vertex* sink, lca* your_lca){
	...
	do {
		...
		else compute_dag_edges(dag_graph, prev_col_edge, col_edge, NULL);
...
Node* last_node_query(Vertex* vertex, Node* u, Node* v , lca* your_lca){
	if (u == v) return v;
		...
		Node* w1 = get_lca(your_lca, v, vertex->p_chain_ccw);
		Node* w2 = get_lca(your_lca, v, vertex->p_chain_cw);
		...
		if (get_lca(your_lca, vertex->h, y) == y)
			return vertex->h;
		else if (x != vertex->h)
			return x;
		else {
			if (vertex->h == vertex->p_chain_ccw)
				return x;
			else if (vertex->h == vertex->p_chain_cw)
				return x;
			else {
				if (get_lca(your_lca, vertex->p_chain_ccw, z) == z)
					return get_lca(your_lca, v, vertex->p_chain_ccw);
				else if (get_lca(your_lca, vertex->p_chain_cw, z) == z)
					return get_lca(your_lca, v, vertex->p_chain_cw);
			}
		}
	}
